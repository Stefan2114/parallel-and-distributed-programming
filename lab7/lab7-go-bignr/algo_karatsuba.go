...
func PolyMulDistributedKaratsuba(p, q []*big.Int, rank, worldSize int) []*big.Int {
...
func polyMulDistributedKaratsubaCoordinator(p, q []*big.Int) []*big.Int {
	lenP := len(p)
	lenQ := len(q)
...
func polyMulDistributedKaratsubaWorker(tag int) []*big.Int {
	p := RecvPoly(0, tag)
	q := RecvPoly(0, tag+1)
	nrThreads := 16
